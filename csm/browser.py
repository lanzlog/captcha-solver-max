...
def browser_kwargs(prefix: str, sticky_key: str | None = None) -> dict:
    """Browser-trust levers, env-configurable per solver.

    prefix is TURNSTILE | RECAPTCHA | HCAPTCHA | CLOUDFLARE. Turnstile defaults
    headless=1; interactive-checkbox solvers default headless=0.

    Proxy precedence:
      1. explicit per-solver static env `{PREFIX}_PROXY`
      2. sticky pool pin when sticky_key set (IP-bound cookies / multi-step)
      3. round-robin next when SOLVER_PROXY_ROTATE=1
    """
    # Turnstile interactive checkbox needs headful+xvfb (start.sh default 0).
    default_headless = "0"
    kw = {"humanize": True,
    ...
    if os.getenv(f"{prefix}_PROXY"):
        ...
        try:
            if sticky_key:
                from csm.proxypool import sticky_proxy
                pinned = sticky_proxy(sticky_key)
                if pinned:
                    kw["proxy"] = pinned
                    log.info("%s sticky proxy key=%r → %s", prefix, sticky_key[:48],
                             pinned.split("@")[-1] if "@" in pinned else pinned)
            if "proxy" not in kw:
                from csm.proxypool import next_proxy
                rotated = next_proxy()
                if rotated:
                    kw["proxy"] = rotated
                    log.info("%s using rotated proxy %s", prefix,
                             rotated.split("@")[-1] if "@" in rotated else rotated)
        except Exception as e:  # pragma: no cover - pool is best-effort
            log.warning("proxy rotation unavailable: %s", e)
    if os.getenv(f"{prefix}_GEOIP") == "1" or (kw.get("proxy") and prefix in ("TURNSTILE", "CLOUDFLARE")):
        kw["geoip"] = True
    return kw
