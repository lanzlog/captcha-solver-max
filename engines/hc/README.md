...
| `vision.py`| Canvas → 4×4 tiles → Mistral vision → click → verify |
| `__init__.py`   | Package exports                                    |
| `README.md`     | This file                                          |

The Mistral `KeyPool` + shared `apikey.txt` and the browser helpers
(selector / pre-actions / post_fetch) now live in `../csm/`
(`csm/mistral.py`, `csm/apikey.txt`, `csm/browser.py`), shared with the
reCAPTCHA solver.
