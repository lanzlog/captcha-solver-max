...
- **Challenge opens → image-solve (Mistral vision):** the image grid opens fine (only
  ...
  pool** (`csm/apikey.txt`, thousands of keys, round-robin + auto-failover), clicks the
...
`csm/mistral.py` `KeyPool` reads `csm/apikey.txt` (one key per line, shared with
...
```python
import asyncio
from engines.rc import (run_rc_v3, run_rc_invisible,
                       run_rc_v2, run_rc_v2_realpage)

asyncio.run(run_rc_v3(sitekey="6Lc...", url="https://target.com", action="submit"))
asyncio.run(run_rc_v2(sitekey="6Lf...", url="https://target.com/form"))
```
...
| `vision.py` | Image-challenge solver (screenshot → tiles → vision → click → verify) |
| `template.html`  | v2 widget page (`.g-recaptcha` + api.js / enterprise.js) |
| `__init__.py`    | Package exports                                    |

The Mistral `KeyPool` and the shared `apikey.txt` now live in `../csm/` (shared with
